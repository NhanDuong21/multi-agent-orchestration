...
Title trim, bắt buộc 1–160 ký tự, từ chối rỗng/trắng/kiểu sai và ký tự NUL (U+0000). Description mặc định chuỗi rỗng khi bỏ qua, phải là string tối đa 4000 ký tự; null không hợp lệ. Validation dùng độ dài chuỗi JavaScript. Phiếu mới luôn open; nếu POST gửi status thì chỉ open được chấp nhận. Không có sửa title/description hoặc xóa phiếu trong Lab 00.

Đổi status giữ id/title/description/createdAt; updatedAt tăng khi status thay đổi. Gửi lại status cũ không đổi updatedAt. ID sai định dạng hoặc không tồn tại trả 404; URL có encoding hỏng trả 400 INVALID_URL trước khi giải mã ID. SQL có tham số, schema CHECK.
